...
uint16_t buf_write_pos = 0;
uint16_t buf_read_pos = 0;
float32_t avg_chirp[NUM_SAMPLES_PER_CHIRP];
float32_t avg_chirp_q[DOWNSAMPLE_CHIRP * Q_DEPTH];
...
static __NO_RETURN void main_task(void *pvParameters)
{
    ...
    for (;;)
    {
        ...
        if (sp_state == FIFO_READ_DONE)
        {
            ...
            // Copy to UDP buffer
            memcpy(&avg_chirp_q[buf_write_pos * DOWNSAMPLE_CHIRP], &avg_chirp[sample_offset], DOWNSAMPLE_CHIRP * sizeof(float32));
            buf_write_pos = (buf_write_pos + 1) % Q_DEPTH;
            ...
            if ((buf_read_pos < buf_write_pos) &&
                (buf_write_pos - buf_read_pos) >= (MAX_UDP_XMIT / (DOWNSAMPLE_CHIRP * sizeof(float32_t))))
            {
                printf("Notifying with read/write pos %i-%i\n", buf_read_pos, buf_write_pos);
                xTaskNotifyGive(udp_server_handle);
            }
            else if (buf_write_pos < buf_read_pos &&
                     ((Q_DEPTH - buf_read_pos) + buf_write_pos) >= (MAX_UDP_XMIT / (DOWNSAMPLE_CHIRP * sizeof(float32_t))))
            {
                printf("................wrap Notifying with read/write pos %i-%i\n", buf_read_pos, buf_write_pos);
                xTaskNotifyGive(udp_server_handle);
            }
