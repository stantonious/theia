...
extern struct inf_result inf_results;
extern int report_class;
...
void udp_server_task(void *arg)
{
    ...
    while (true)
    {
        ...
        if (client_connected)
        {
            result = cy_socket_sendto(server_handle, &inf_results, sizeof(struct inf_result), CY_SOCKET_FLAGS_NONE,
...
cy_rslt_t udp_server_recv_handler(cy_socket_t socket_handle, void *arg)
{
    ...
    sscanf(message_buffer, "%d-%d", &report_class,&sample_offset);
